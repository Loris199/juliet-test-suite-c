...
# INFER
USE_INFER = int(os.environ.get("USE_INFER", "0"))
INFER_RESULTS_DIR = os.environ.get("INFER_RESULTS_DIR", root_dir + "/infer-out")
INFER_CC = "/home/loris/Desktop/Installations/infer-linux-x86_64-v1.2.0/lib/infer/infer/lib/wrappers/cc"
...
def generate(path, output_dir, keep_going):
    ...
    if USE_INFER == 1:
        retcode = subprocess.Popen(["cmake", "-DCMAKE_C_COMPILER=" + INFER_CC, "-DOUTPUT_DIR:STRING=" + output_dir, "."], cwd=path).wait()
    else:
...
def make(path, keep_going):
    ...
    if USE_INFER == 1:
        if keep_going:
            subprocess.Popen(["infer", "capture", "--results-dir", INFER_RESULTS_DIR, "--", "make", "-j16", "-k"], cwd=path).wait()
        else:
            retcode = subprocess.Popen(["infer", "capture", "--results-dir", INFER_RESULTS_DIR, "--", "make", "-j16"], cwd=path).wait()
            if retcode != 0:
